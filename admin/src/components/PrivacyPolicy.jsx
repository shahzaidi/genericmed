...
import {
  createPrivacyPolicyAction,
  getPrivacyPolicyPageDetails,
  updatePrivacyPolicyPageAction,
} from "../redux/actions/digitalMarketing/privacyPageActions";
import { useFormik } from "formik";
import { useDispatch, useSelector } from "react-redux";
import {
  privacyPolicyInitialValues,
  privacyPolicyValidationSchema,
} from "../common/digitalMarketing/privacyPolicyValidation";
import { getPackageAndAuthenticityPageDetails } from "../redux/actions/digitalMarketing/packageAndAuthenticityActions";
...
const PrivacyPolicy = () => {
  ...
  const { loading, privacyPolicy, error } = useSelector(
    (state) => state.getPrivacyAndPolicyDetailsReducer
  );
  ...
  const handlePrivacyPolicy = () => {
    ...
    if (Object.keys(formik.errors).length === 0) {
      dispatch(createPrivacyPolicyAction(formik?.values));
    }
  };

  const updatePackagingAndAuthenticityPageData = () => {
    ...
    if (
      ...
      dispatch(
        updatePrivacyPolicyPageAction(privacyPolicy?._id, formik?.values)
      );
    }
  };
  ...
  useEffect(() => {
    if (privacyPolicy && Object.keys(privacyPolicy).length >= 1) {
      formik?.setFieldValue("slug", privacyPolicy?.slug);
      formik?.setFieldValue("metaDescription", privacyPolicy?.metaDescription);
      formik?.setFieldValue("metaTitle", privacyPolicy?.metaTitle);
      formik?.setFieldValue("metaKeyword", privacyPolicy?.metaKeyword);
    }
  }, [privacyPolicy]);
  ...
  const formik = useFormik({
    ...
    onSubmit: privacyPolicy?._id
      ? updatePackagingAndAuthenticityPageData
      : handlePrivacyPolicy,
          ...
          <div className="header-admin">
            ...
            {loading ? (
              ...
              <div className="produche">
                ...
                {formik && Object.keys(privacyPolicy).length !== 0 ? (
