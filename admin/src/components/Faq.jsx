...
import {
  faqInitialValues,
  faqValidationSchema,
} from "../common/digitalMarketing/faqValidation";
import {
  faqPageAction,
  getFaqPageDetails,
  updateFaqAction,
} from "../redux/actions/digitalMarketing/faqActions";
...
const Faq = () => {
  ...
  const { loading, faq, error } = useSelector(
    (state) => state.getFaqDetailsReducer
  );
  ...
  const handleFaq = () => {
    ...
    if (Object.keys(formik.errors).length === 0) {
      dispatch(faqPageAction(formik?.values));
    }
  };

  const updateFaqPageData = () => {
  ...
  useEffect(() => {
    dispatch(getFaqPageDetails());
  }, [dispatch]);

  useEffect(() => {
    if (faq && Object.keys(faq).length >= 1) {
      formik?.setFieldValue("slug", faq?.slug);
      formik?.setFieldValue("metaDescription", faq?.metaDescription);
      formik?.setFieldValue("metaTitle", faq?.metaTitle);
      formik?.setFieldValue("metaKeyword", faq?.metaKeyword);
    }
  }, [faq]);
  ...
  const formik = useFormik({
    ...
    onSubmit: faq?._id ? updateFaqPageData : handleFaq,
          ...
          <div className="header-admin">
            ...
            {loading ? (
              ...
              <div className="produche">
                ...
                {formik && Object.keys(faq).length !== 0 ? (
