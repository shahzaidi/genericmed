...
} from "../common/digitalMarketing/contactUsValidation";
import {
  contactUsPageAction,
  getContactUsPageDetails,
  updateContactUsPageAction,
} from "../redux/actions/digitalMarketing/contactUsActions";
...
const ContactUs = () => {
  ...
  const { loading, contactUs, error } = useSelector(
    (state) => state.getContactUsPageDetailsReducer
  );
  ...
  const handleContactUsPage = () => {
    ...
    if (Object.keys(formik.errors).length === 0) {
      dispatch(contactUsPageAction(formik?.values));
    }
  };

  const updateContactUsPageData = () => {
    console.log(formik?.values, "formikValues....//");
    if (
      Object.keys(formik.errors).length === 0 &&
      Object.keys(contactUs).length !== 0
    ) {
      dispatch(updateContactUsPageAction(contactUs?._id, formik?.values));
    }
  };

  useEffect(() => {
    dispatch(getContactUsPageDetails());
  }, [dispatch]);

  useEffect(() => {
    if (contactUs && Object.keys(contactUs).length >= 1) {
      formik?.setFieldValue("slug", contactUs?.slug);
      formik?.setFieldValue("metaDescription", contactUs?.metaDescription);
      formik?.setFieldValue("metaTitle", contactUs?.metaTitle);
      formik?.setFieldValue("metaKeyword", contactUs?.metaKeyword);
    }
  }, [contactUs]);
  ...
  const formik = useFormik({
    ...
    onSubmit: contactUs?._id ? updateContactUsPageData : handleContactUsPage,
          ...
          <div className="header-admin">
            ...
            {loading ? (
              ...
              <div className="produche">
                ...
                {formik && Object.keys(contactUs).length !== 0 ? (
