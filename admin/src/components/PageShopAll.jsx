...
import { aboutUsInitialValues } from "../common/digitalMarketing/aboutUsValidation";
import {
  shopAllPageInitialValues,
  shopAllPageValidationSchema,
} from "../common/pagesFolder/shopAllPageValidations";
import { useDispatch, useSelector } from "react-redux";
import {
  createShopAllPagePageAction,
  getShopAllPagePageDetailsAction,
  updateShopAllPagePageAction,
} from "../redux/actions/pagesFolder/shopAllPageActions";
import { uploadImagesAction } from "../redux/actions/imagesActions";
import { updateShopAllPageAction } from "../redux/actions/digitalMarketing/shopAllActions";
...
const PageShopAll = () => {
  ...
  const { loading, shopAllPage, error } = useSelector(
    (state) => state.getShopAllPagePageDetailsReducer
  );
  ...
  const handleShopAllPage = () => {
    console.log(formik?.values, formik?.errors, "formikValues....//");
    if (Object.keys(formik.errors).length === 0) {
      dispatch(createShopAllPagePageAction(formik?.values));
    }
  };

  const updateShopAllPage = () => {
    console.log(formik?.values, "formikValues....//");
    if (
      Object.keys(shopAllPage).length !== 0 &&
      Object.keys(formik.errors).length === 0
    ) {
      dispatch(updateShopAllPagePageAction(shopAllPage?._id, formik?.values));
    }
  };

  const formik = useFormik({
    initialValues: shopAllPageInitialValues,
    // onSubmit: id ? updateProduct : handleHomePage,
    onSubmit: shopAllPage?._id ? updateShopAllPage : handleShopAllPage,
    validationSchema: shopAllPageValidationSchema,
  });
  ...
  useEffect(() => {
    if (shopAllPage && shopAllPage?.sliderBannerImageUrl?.length >= 1) {
      formik.setFieldValue(
        "sliderBannerImageUrl",
        shopAllPage?.sliderBannerImageUrl
      );
    }
  }, [shopAllPage]);

  useEffect(() => {
    dispatch(getShopAllPagePageDetailsAction());
  }, [dispatch]);
  return (
    <div>
      <div className="admin-dashboard">
        ...
        <div className="rightt-panel">
          <Header />
          <Layout heading="Shop All"></Layout>
          {loading ? (
      ...
      <button className='titlho'>  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="19">
                ...
                {formik && Object.keys(shopAllPage)?.length !== 0 ? (
