...
import {
  ...
  blogPageValidationSchema,
} from "../common/digitalMarketing/blogValidation";
import { useDispatch, useSelector } from "react-redux";
import {
  createBlogPage,
  getBlogPageDetails,
  updateBlogPageAction,
} from "../redux/actions/digitalMarketing/blogPageActions";
...
const BlogPage = () => {
  ...
  const { loading, blogPage, error } = useSelector(
  ...
  const handleBlogPage = () => {
    ...
    if (Object.keys(formik.errors).length === 0) {
      dispatch(createBlogPage(formik?.values));
    }
  };

  const updateBlogPageData = () => {
    console.log(formik?.values, "formikValues....//");
    if (
      Object.keys(formik.errors).length === 0 &&
      Object.keys(blogPage).length !== 0
    ) {
      dispatch(updateBlogPageAction(blogPage?._id, formik?.values));
    }
  };
  ...
  useEffect(() => {
    if (blogPage && Object.keys(blogPage).length >= 1) {
      formik?.setFieldValue("slug", blogPage?.slug);
      formik?.setFieldValue("metaDescription", blogPage?.metaDescription);
      formik?.setFieldValue("metaTitle", blogPage?.metaTitle);
      formik?.setFieldValue("metaKeyword", blogPage?.metaKeyword);
    }
  }, [blogPage]);
  ...
  const formik = useFormik({
    ...
    onSubmit: blogPage?._id ? updateBlogPageData : handleBlogPage,
    validationSchema: blogPageValidationSchema,
  });
          ...
          <div className="header-admin">
            ...
            {loading ? (
              ...
              <div className="produche">
                ...
                {formik && Object.keys(blogPage).length !== 0 ? (
