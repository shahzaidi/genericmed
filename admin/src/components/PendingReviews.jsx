...
import {
  approvedReviewAction,
  deleteGuestUsersReview,
  getAllGuestUsersPendingReviews,
...
const PendingReviews = () => {
  ...
  const pendingReviewsPerPageCount = Number(10);
  const { reviewsLoading, reviews, pendingReviewsCount, reviewsError } =
    useSelector((state) => state.getAllGuestUsersPendingReviewsReducer);
  ...
  useEffect(() => {
    dispatch(getAllGuestUsersPendingReviews(currentPage));
  }, [currentPage]);
  ...
  const handleStatusChange = (reviewId, comment, name) => {
    let confirm = window.confirm(
      `Are you sure, wants to approve this review:- ${comment} `
    );
    if (confirm) {
      dispatch(approvedReviewAction(reviewId, name));
    } else {
      return;
    }
  };
  ...
  return (
    <div className="admin-dashboard">
      ...
      <div className="rightt-panel">
        <Header />
        <Layout heading="Pending Reviews"></Layout>
          ...
          {reviews?.length >= 1 && (
            <div className="pagination-innfo">
              <ReactPaginate
                ...
                breakLabel={"..."}
                pageCount={Math.ceil(
                  Math.max(0, pendingReviewsCount) / pendingReviewsPerPageCount
                )}
