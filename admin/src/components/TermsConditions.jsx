...
import {
  packageAndAuthenticityInitialValues,
  packageAndAuthenticityValidationSchema,
} from "../common/digitalMarketing/packageAndAuthenticityValidation";
import {
  getPackageAndAuthenticityPageDetails,
  packageAndAuthenticityAction,
  updatePackageAndAuthenticityPageAction,
} from "../redux/actions/digitalMarketing/packageAndAuthenticityActions";
...
const TermsConditions = () => {
  ...
  const { loading, packagingAndAuthenticity, error } = useSelector(
    (state) => state.getPackageAndAuthenticityDetailsReducer
  );
  ...
  const handlePackageAndAuthenticity = () => {
    console.log(formik?.values, formik?.errors, "formikValues....//");
    if (Object.keys(formik.errors).length === 0) {
      dispatch(packageAndAuthenticityAction(formik?.values));
    }
  };

  const updatePackageAndAuthenticityPageData = () => {
    console.log(formik?.values, "formikValues....//");
    if (
      Object.keys(formik.errors).length === 0 &&
      Object.keys(packagingAndAuthenticity).length !== 0
    ) {
      dispatch(
        updatePackageAndAuthenticityPageAction(
          packagingAndAuthenticity?._id,
          formik?.values
        )
      );
    }
  ...
  useEffect(() => {
    dispatch(getPackageAndAuthenticityPageDetails());
  }, [dispatch]);

  useEffect(() => {
    if (
      packagingAndAuthenticity &&
      Object.keys(packagingAndAuthenticity).length >= 1
    ) {
      formik?.setFieldValue("slug", packagingAndAuthenticity?.slug);
      formik?.setFieldValue(
        "metaDescription",
        packagingAndAuthenticity?.metaDescription
      );
      formik?.setFieldValue("metaTitle", packagingAndAuthenticity?.metaTitle);
      formik?.setFieldValue(
        "metaKeyword",
        packagingAndAuthenticity?.metaKeyword
      );
    }
  }, [packagingAndAuthenticity]);

  const formik = useFormik({
    initialValues: packageAndAuthenticityInitialValues,
    // onSubmit: id ? updateProduct : handleHomePage,
    onSubmit: packagingAndAuthenticity?._id
      ? updatePackageAndAuthenticityPageData
      : handlePackageAndAuthenticity,
    validationSchema: packageAndAuthenticityValidationSchema,
  });
