...
import {
  ...
  getAllGuestUsersApprovedReviews,
...
const ApproveReviews = () => {
  ...
  const approvedReviewsPerPageCount = Number(10);
  const { reviewsLoading, reviews, approvedReviewsCount, reviewsError } =
    useSelector((state) => state.getAllGuestUsersApprovedReviewsReducer);
  ...
  const handleDeleteReview = (productId, reviewId, comment, name) => {
  ...
  useEffect(() => {
    dispatch(getAllGuestUsersApprovedReviews(currentPage));
  }, [currentPage]);
  ...
  return (
    <div className="admin-dashboard">
      ...
      <div className="rightt-panel">
        <Header />
        <Layout heading="Approve Reviews"></Layout>
              ...
              {reviews?.length <= 0 ? (
                ...
                reviews.map((review, index) => (
                  <tr key={index} className="bl">
                    ...
                    <td>
                      {/* <FontAwesomeIcon icon={faCheckCircle} className="approved-icon" onClick={() => handleStatusChange(category.id, 'Approved')} /> */}
          ...
          {reviews?.length >= 1 && (
            <div className="pagination-innfo">
              <ReactPaginate
                ...
                breakLabel={"..."}
                pageCount={Math.ceil(
                  Math.max(0, approvedReviewsCount) /
                    approvedReviewsPerPageCount
                )}
